...
export default function Review(item) {
  ...
  const urlZip = `https://babzip.seunga.shop/reviews/zip/${item.reviewId}`;
  const urlDelZip = `https://babzip.seunga.shop/reviews/deleteZip/${item.reviewId}`;
  ...
  function clickLike() {
    if (!likeState) {
      api.post(urlZip).catch(function (error) {
        console.log(error.message);
      });
      sethowManyLike(++value);
      setLike(!likeState);
    } else {
      api.delete(urlDelZip).catch(function (error) {
        console.log(error.message);
      });
      sethowManyLike(--value);
      setLike(!likeState);
  ...
  return (
    ...
    >
      ...
      >
        <div className="contents-review" style={{ position: "absolute" }}>
          ...
          <div className="topReview-review">
            ...
            <p className="userDetail-review">
              {item.nickname}({formattedDate})
            </p>
          </div>
        </div>
      </Link>
      <div
        ...
        style={{ position: "absolute", left: "375px" }}
      >
        {likeState ? (
        ...
        <p
          className="likeTxt-review"
          style={likeState ? { color: "red" } : { color: "white" }}
        >
          {howManyLike}
        </p>
      </div>
