  ...
  return (
    <div className='myReview-ManageReview'>
      <div className='deleteBar-myReview'>
        <p className='textSelec-deleteBar' onClick={()=>{setSelState(!selecState); setSelBtn([]);}}>{selecState? "취소" : "선택"}</p>
        <p className='textDel-deleteBar' style={{color:"#767676", textDecoration:'underline'}}
         onClick={()=>{setDel(selectBtn); if(selectBtn.length!==0) setModal(true);}}>삭제하기</p>
      </div>
      <div className='reviews-MyReview'>
        {reviewData.length?<div className='result-MyReview'>
              {reviewData.map((item)=>{
                return(
                  <div className='container-MyReview' key={item.reviewId}>
                    {/* 기본정보 */}
                    <div className='basicInfo-MyReview' style={{position:'relative'}} >
                      <img className='selectBtn-MyReview' src={checkSelect(item.reviewId)?srcSelActBtn:srcSelBtn} alt=""
                       onClick={()=>{clickSelect(item.reviewId)}}
                       style={selecState?{display:'block'}:{display:'none'}}/>
                      <img src={item.imageUrls[0]} alt="" style={{width:'128px', height:'129px', borderRadius:'10px 0px 00px 10px', backgroundColor:'#D9D9D9'}}/>
                      ...
                      <div className='btn-MyReview'>
                        <img src={scrDeletBtn} alt="" style={{width:'31px', height:'31px'}} onClick={()=>{setDel([item.reviewId]); setModal(true);}}/>
                        <div className='sectionDetail-Btn' style={{width:'31px', height:'31px'}} onClick={()=>{clickDetail(item.reviewId)}}>
                          <img src={scrDetailBtn} alt="" style={{width:'16px', height:'7px'}} />
                        </div>
                      </div>
                    </div>
                    {/* 상세정보 */}
                    <div className='moreInfo-MyReview' style={checkDetail(item.reviewId)?{display:'block'}:{display:'none'}}>
      ...
      {/* 모달 */}
      <div className='modalDel-MyReview' style={viewModal?{display:'flex'}:{display:'none'}}>
          <div className='container-modalDel'>
              <p className='title-modalDel'>선택한 나의 리뷰를 정말로 삭제하시겠어요?</p>
              <div className='btn-modalDel'>
                <button className='cancel-modalDel' onClick={()=>{setModal(false)}}>취소</button>
                <button className='confirm-modalDel' style={{color:"red"}} onClick={()=>{ delReview(); setModal(false);}}>삭제하기</button>
              </div>
          </div>
      </div>
    </div>
