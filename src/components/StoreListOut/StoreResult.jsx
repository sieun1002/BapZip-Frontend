...
export default function StoreResult(props) {
  ...
  const urlget = `https://babzip.seunga.shop/stores/list/${menuListApi[menu]}?schoolId=${storedUserId}`;
  function getStore() {
    api
      .get(urlget)
      .then(function (response) {
        setData(
          response.data.result.filter(
            (item) => item.inOut === "OUT" && item.name.includes(props.input)
          )
        );
      })
    ...
    <div className="StoreResult-StoreListIn">
      <div className="content-StoreResult">
        ...
        <div className="menubar-StoreResult">
          ...
          <div
            className="menuList-StoreResult"
            style={menuBar ? { display: "block" } : { display: "none" }}
          >
            <p
              className="menu-menuList"
              style={menu === 0 ? { color: "black", fontWeight: "600" } : {}}
              onClick={() => {
                clickMenu(0);
              }}
            >
            ...
            <p
              className="menu-menuList"
              style={menu === 1 ? { color: "black", fontWeight: "600" } : {}}
              onClick={() => {
                clickMenu(1);
              }}
            >
            ...
            <p
              className="menu-menuList"
              style={menu === 2 ? { color: "black", fontWeight: "600" } : {}}
              onClick={() => {
                clickMenu(2);
              }}
            >
                ...
                >
                  ...
                  >
                    <div className="detailReview-MyReview">
                      ...
                      <p className="date-StoreResult">
                        {item.category}ㆍ{item.inOut === "IN" ? "교내" : "교외"}
                      </p>
                      <div className="rating-MyReview">
                        ...
                        <p className="reviewCount-rating">
                          ({item.reviewCount})
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="btn-MyReview">
                    <img
                      src={item.isMyZip ? scrBookmarkBtn : scrEmBookmarkBtn}
                      alt=""
                      style={{ width: "31px", height: "31px" }}
                      onClick={() => {
                        clickBookmark(item.storeId, item.isMyZip);
                      }}
                    />
                  </div>
                </div>
